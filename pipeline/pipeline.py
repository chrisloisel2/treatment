...
# ── Chemins fixes ─────────────────────────────────────────────────────────────
INGEST_DIR   = Path("/mnt/storage_nfs/ingest")  # source ET espace de travail (déposé par l'opérateur)
...
if __name__ == "__main__":
    ...
    top = argparse.ArgumentParser(
        description=(
            ...
            "\nSous-commandes :\n"
            "  run        — traite une ou toutes les sessions de /mnt/ingest\n"
    ...
    p_run.add_argument("--delete-after-store", action="store_true", default=False,
                       help="Supprimer de /mnt/ingest après copie (requiert --write)")
    ...
    if not INGEST_DIR.exists():
        run_errors.append(f"/mnt/ingest non accessible : {INGEST_DIR}")
