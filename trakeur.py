...
def detect_head(blocks):
    ...
    - moins mobile
    - moins dispersé
    - plus central par rapport aux deux autres
    - plus extrême sur un axe "hauteur" candidat
    """
    ...
    # choisir le meilleur axe "hauteur" sans supposer x/y/z
    best_heights = None
    best_sep = -np.inf
    for axis in range(3):
        for sign in (-1, 1):
    ...
    score = (
        3.0 * rank_points(motions, higher_better=False) +
        2.0 * rank_points(spreads, higher_better=False) +
        3.0 * rank_points(pair_mean, higher_better=False) +
        1.0 * rank_points(best_heights, higher_better=True)
    )
