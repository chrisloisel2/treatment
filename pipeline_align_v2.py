...
def align_session(session_path, validate_only=False):
    ...
    # Décalage fin appliqué aux timestamps caméra pour tester un alignement plus précis
    frame_times_ns_adjusted = frame_times_ns + (FINE_OFFSET_MS * 1_000_000)
    ...
    for col in all_tracker_cols:
        ...
        result[col] = np.interp(frame_times_ns_adjusted, tracker_times_ns, vals)
    ...
    report["cameras"][cam] = {
        "status": align_status,
        "estimated_offset_ms": float(mean_offset_ms),
        "fine_offset_ms": float(FINE_OFFSET_MS),
...
Exemples:
  ...
  python3 pipeline_aall_tracker_colslign.py ~/Desktop --batch --validate-only
        """,
    )
